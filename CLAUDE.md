...
- Prisma + Postgres. Local dev via Docker Compose; production target is Vercel + Neon.
- Connection strings: runtime queries (`lib/prisma.ts`, via the `@prisma/adapter-pg` driver adapter) use `DATABASE_URL` — Neon's pooled connection on Vercel, since serverless functions can fan out to many concurrent connections. The Prisma CLI (migrate/studio/seed, configured in `prisma.config.ts`) uses `DATABASE_URL_UNPOOLED` — a direct connection, because migrations run session-level locks/DDL that Neon's transaction-mode pooler doesn't support reliably — falling back to `DATABASE_URL` when unset, so local dev (no pooler distinction) only needs one variable. `schema.prisma`'s `datasource` block deliberately has no `url` at all in Prisma 7 with a driver adapter — neither runtime nor CLI reads a connection string from there.
